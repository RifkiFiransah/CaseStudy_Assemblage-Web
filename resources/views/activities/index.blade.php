  ...
  <div class="section-header">
    ...
    <div class="section-header-breadcrumb">
      <div class="breadcrumb-item">
        <a href="{{ route('calendar') }}" class="btn btn-primary"><i class="far fa-calendar-alt"></i> Lihat Kalender</a>
      </div>
    </div>
  </div>
  <div class="section-body">
    <h2 class="section-title">{{ date('d - F - Y') }}</h2>
    <div class="row">
      <div class="col-12">
                ...
                <div class="d-flex justify-content-between mt-4">
                  <span class="mr-3">
                    <img src="{{ asset('./assets/img/avatar/'. $proker->users->profile) }}"
                        alt="{{ $proker->users->name }}" class="rounded-circle object-cover mr-2"
                        width="20"
                        data-toggle="tooltip"
                        data-placement="top">
                    {{ $proker->users->name }}
          ...
          @empty
            <div class="activity">
              <div class="activity-detail">
                <p>Data Program Kerja belum tersedia, silahkan tambahkan data dan selesaikan Program Kerja supaya dapat dilihat timeline nya.</p>
              </div>
            </div>
          @endforelse
        </div>
...
@foreach($tasks as $proker)
  let span_{{ $loop->iteration }} = document.querySelector('#date-{{ $proker->id }}')
  let date_{{ $loop->iteration }} = moment("{{ $proker->tanggal }}")
  span_{{ $loop->iteration }}.textContent = date_{{ $loop->iteration }}.fromNow()
@endforeach
</script>
@endpush